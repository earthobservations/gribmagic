...
from gribmagic.unity.modules.config.constants import KEY_VARIABLES, KEY_FORECAST_STEPS, \
    KEY_URL_PATH, KEY_URL_BASE, KEY_URL_FILE, KEY_INITIALIZATION_DATE_FORMAT, KEY_FILE_POSTFIX
...
def test_build_model_file_lists():
    to_test = build_model_file_lists(WeatherModels.DWD_ICON_EU, 0, datetime(2020, 6, 10).date())
    assert to_test == \
           {'local_file_paths': [Path('/app/data/dwd-icon-eu_20200610_00_air_temperature_2m_000.grib'),
                                 Path('/app/data/dwd-icon-eu_20200610_00_air_temperature_2m_001.grib')],
            'remote_file_paths': ['http://foobar.example.org/test_remote_dir/00/t_2m/test_remote_file_single-level_2020061000_000_T_2M.grib2.bz2',
                                  'http://foobar.example.org/test_remote_dir/00/t_2m/test_remote_file_single-level_2020061000_001_T_2M.grib2.bz2'],
            'local_store_file_paths': [Path('/app/data/dwd-icon-eu/20200610_00/air_temperature_2m.nc')]
            }
